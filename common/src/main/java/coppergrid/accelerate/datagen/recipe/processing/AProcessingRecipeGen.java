...
public abstract class AProcessingRecipeGen extends ARecipeProvider {
    ...
    <T extends ProcessingRecipe<?>> GeneratedRecipe create(Supplier<ItemLike> singleIngredient,
        ...
        return create(Accelerate.MOD_ID, singleIngredient, transform);
    }
    ...
    <T extends ProcessingRecipe<?>> GeneratedRecipe create(String name,
                                                           UnaryOperator<ProcessingRecipeBuilder<T>> transform) {
        return create(Accelerate.asResource(name), transform);
    }
    ...
    protected Supplier<ResourceLocation> idWithSuffix(Supplier<ItemLike> item, String suffix) {
        return () -> {
            ResourceLocation registryName = CatnipServices.REGISTRIES.getKeyOrThrow(item.get()
                    .asItem());
            return Accelerate.asResource(registryName.getPath() + suffix);
        };
    }

    @Override
    public @NotNull String getName() {
        return "Accelerate's Processing Recipes: " + getRecipeType().getId()
                .getPath();
    }
}